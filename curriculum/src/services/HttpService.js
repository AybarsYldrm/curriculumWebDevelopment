...
class HttpService {
  ...
  sendJson(res, statusCode, payload, extraHeaders = {}) {
    ...
    const headers = Object.assign({
      ...
      'Access-Control-Allow-Origin': this.allowedOrigins.join(','),
  ...
  sendGraph(res, statusCode, raw, extraHeaders = {}) {
    if (res.headersSent) return;
    // Graph responses are JSON objects in this stack
    const headers = Object.assign({ 'Content-Type':'application/json; charset=utf-8' }, this.securityHeaders, extraHeaders);
    res.writeHead(statusCode, headers);
   ...
   */
  sendHtml(res, statusCode, html, nonce = null, extraHeaders = {}) {
    if (res.headersSent) return;

    // build secure CSP that restricts scripts and styles to our nonce
    // note: img-src and other directives can be extended as needed
    let csp = `default-src 'self'; base-uri 'self'; frame-ancestors 'none'`;
    if (nonce) {
      csp += `; script-src 'nonce-${nonce}' 'strict-dynamic'`; // 'strict-dynamic' helps with trusted script loaders
      csp += `; style-src 'nonce-${nonce}'`; // inline style attributes are *not* covered by nonce
      csp += `; object-src 'none'`;
  ...
  async handleRequest(req, res) {
    try {
      ...
      if (route.options && route.options.auth) {
        ...
        const token = authHeader.startsWith('Bearer ') ? authHeader.slice(7) : (this._getCookie(req, 'access_token') || authHeader || '');
        if (!token) {
        ...
        // `verifyJWT` in your AuthService should return { payload, user } or user object
